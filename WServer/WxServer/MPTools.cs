...
namespace XJWZCatering.WXHelper
{
    /// <summary>
    /// 处理微信小程序用户数据的签名验证和解密
    /// </summary>
    public class WeChatAppDecrypt
    {
        ...
        {
            var appId = ConfigurationManager.AppSettings["appid_vip"].ToString();
            var appSecret = ConfigurationManager.AppSettings["secret_vip"].ToString();
